...
if($RequireUpgradeEvidence){
    $path=Join-Path $root $UpgradeEvidence
    if(-not(Test-Path -LiteralPath $path)){$failures.Add("missing upgrade evidence: $UpgradeEvidence")}
    else{$e=[IO.File]::ReadAllText($path,[Text.Encoding]::UTF8)|ConvertFrom-Json;if($e.fromVersion-ne'2.2.4'-or-not$e.apkUpgrade-or-not$e.databaseIntegrity-or-not$e.projectIntegrity-or-not$e.mediaIntegrity-or[string]::IsNullOrWhiteSpace([string]$e.device.serial)){$failures.Add('2.2.4 to current device upgrade evidence is incomplete')}}
}
