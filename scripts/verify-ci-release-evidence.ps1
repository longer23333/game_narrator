...
$failures=[Collections.Generic.List[string]]::new();if($e.status-ne'passed'){$failures.Add('CI status is not passed')};if($e.commit-ne$commit){$failures.Add("CI commit mismatch: $($e.commit) != $commit")}
foreach($gate in @('migrationBaseline','documentation','frontend','android','androidEmulator','backend','performanceBaseline')){if($e.gates.$gate-ne'passed'){$failures.Add("CI gate not passed: $gate")}}
