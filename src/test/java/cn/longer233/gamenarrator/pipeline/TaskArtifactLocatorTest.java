...
class TaskArtifactLocatorTest {
    @Test
    void resolvesLatestLiveArtifactWithoutTaskColumnFallback() {
        JdbcDataSource source = new JdbcDataSource();
        source.setURL("jdbc:h2:mem:artifact-locator-" + UUID.randomUUID() + ";DB_CLOSE_DELAY=-1");
        JdbcTemplate jdbc = new JdbcTemplate(source);
        jdbc.execute("CREATE TABLE video_tasks(id UUID PRIMARY KEY,project_id UUID)");
        ...
        jdbc.update("INSERT INTO video_tasks(id,project_id) VALUES(?,?)", taskId, projectId);
        TaskArtifactLocator locator = new TaskArtifactLocator(jdbc);

        assertThat(locator.latest(taskId, "RENDERED_VIDEO")).isEmpty();
        ...
        assertThat(locator.latestAll(taskId)).containsEntry("RENDERED_VIDEO",
                Path.of("indexed.mp4").toAbsolutePath().normalize());
