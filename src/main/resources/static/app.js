...
async function ensureViewScripts(view) {
  if (view === 'import' || view === 'assets') await loadLazyScript(view);
}
...
if (localStorage.getItem('gameNarrator.lastSeenRelease') === '2.2.48') document.querySelector('#updates-open')?.classList.remove('has-update');
...
const guideSteps = [
  ...
  {selector: '.topbar-actions', title: '完成、诊断与再次查看', text: '任务完成后在详情中预览并导出 MP4。任何阶段失败时先查看任务详情和诊断日志；本引导可以随时从“使用引导”重新打开。当前版本为 v2.2.48。'}
];
