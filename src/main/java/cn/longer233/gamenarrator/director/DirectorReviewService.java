...
public class DirectorReviewService {
 ...
 public DirectorReviewView start(UUID taskId,int requestedRounds){
  ...
  var board=workspace.storyboard(taskId); UUID reviewId=UUID.randomUUID(); OffsetDateTime started=OffsetDateTime.now();
  ...
  try{
   String facts=mapper.writeValueAsString(Map.of("confirmedEvents",confirmed,"storyboard",board));
 ...
 private String moderatorPrompt(String facts,List<JsonNode> opinions)throws Exception{return "你是总导演主持人。根据事实和四类评审意见进行分歧与共识检查。事实："+facts+"\n意见："+mapper.writeValueAsString(opinions)+"\n只返回JSON：{summary,consensusScore:0到1,agreements:[],disagreements:[],finalDecisions:[{type,target,action,reason,evidence}],eventIds:[],clipIndexes:[],knowledgeRefs:[],requiresUserConfirmation:true}。最终决定必须可执行且不得直接应用。";}
