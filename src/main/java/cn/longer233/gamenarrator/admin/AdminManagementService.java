...
public class AdminManagementService {
    ...
    public AdminPage syncItems(String search,String status,int page,int size) {
        ...
        List<Map<String,Object>> items=rows("SELECT s.id,u.username,s.item_type,s.local_id,s.object_key,s.size_bytes,s.sync_status,s.attempt_count,s.last_error,s.updated_at,s.synced_at FROM cloud_sync_item s JOIN app_user u ON u.id=s.user_id"+where+"ORDER BY s.updated_at DESC LIMIT ? OFFSET ?",append(args,spec.size(),spec.offset()));
        return AdminPage.of(items,total,spec.page(),spec.size());
    }
    ...
    @Transactional public void retrySync(UUID id) { requireAdmin(); int changed=jdbc.update("UPDATE cloud_sync_item SET sync_status='PENDING',last_error=NULL,next_attempt_at=NULL,updated_at=? WHERE id=? AND sync_status IN ('FAILED','LOCAL_ONLY')",OffsetDateTime.now(ZoneOffset.UTC),id); if(changed==0) throw new IllegalArgumentException("同步项不存在或当前状态不可重试"); audit("RETRY_CLOUD_SYNC","CLOUD_SYNC",id,Map.of("status","PENDING")); }
