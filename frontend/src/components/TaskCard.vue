...
<template>
  <div class="task-card" :data-task-id="task.id" role="button" tabindex="0" :aria-label="`查看任务 ${task.name} 的详情`">
    <div class="task-head"><strong>{{ task.name }}</strong><div class="task-card-actions"><span>{{ task.status }}</span><button type="button" class="task-card-open" :data-open-list-task="task.id">查看与操作</button><button type="button" class="task-card-rename" :data-rename-list-task="task.id" :data-task-name="task.name">改名</button><button type="button" class="task-card-delete" :data-delete-list-task="task.id" :data-task-name="task.name" :aria-label="`将任务 ${task.name} 移入回收站`">回收</button></div></div>
