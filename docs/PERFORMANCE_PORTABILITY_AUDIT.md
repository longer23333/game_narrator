...
- Event-driven frame sampling now augments duration-aware coverage with bounded, deduplicated scene-change and audio peak/clipping anchors. FFmpeg extracts configurable pre/center/post windows, while first-pass visual results select only valuable windows for a bounded second pass; sampling counts and mode are stored in the visual analysis artifact.
...
- 截图搜索上传上限固定为 10 MiB，服务端在读取内容前拒绝超限请求。
