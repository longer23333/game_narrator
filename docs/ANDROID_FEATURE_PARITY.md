...
核心功能基线由 `docs/ANDROID_CORE_BASELINE.json` schema v3 机器校验，分别记录软件功能、自动化验收、可选真机兼容和生产签名状态。当前软件能力状态为 100%（18 项 full），自动化验收已由提交 `368931f` 的 API 29/35 阻断矩阵和 2.2.4→当前持久化 AVD 升级作业证明；真机兼容仍是可选增强且尚未验证，模拟器结果也不冒充 OEM、硬件编解码或生产密钥证明。Web、Windows 与 Android 从 2.2.4 起共享同一语义版本。
...
| 平台导入与下载任务 | 已完成（自动化协议验收） | HTTP 回放和故障注入覆盖 200、206、416、403、429、500、缺失 Content-Length、不支持/错误 Range、连接中断和超时；登录回放覆盖等待、确认、过期和拒绝。提交 `368931f` 的 API 29/35 阻断矩阵已通过；真实账号与真机四平台流程由 `-RequireDeviceEvidence` 独立校验并保持可选，当前不宣称真实账号或 OEM 真机兼容已经验证 |
...
| 软件升级与发行自动化门禁 | 已完成（自动化软件验收） | 提交 `368931f` 的 CI 在同一持久化 API 29 AVD 上从仓库 `v2.2.4` APK 覆盖安装当前 APK，并通过数据库迁移测试和独立持久化哨兵复验应用数据；正式构建脚本仍强制生产密钥并归档 APK、SHA-256、证书指纹及 R8 资料。`productionSigningValidated=false` 与 `physicalDeviceValidated=false` 继续独立记录，不宣称生产签名或 OEM 真机兼容已经验证 |

## 当前优先补齐项

1. 独立发布状态：生产签名密钥仍未验证；正式发布前必须取得 `verify-android-production-release.ps1 -RequireProductionArtifacts` 通过证据。
2. 可选兼容增强：真实账号四平台流程和 OEM/硬件编解码真机兼容仍未验证，保持 `physicalDeviceValidated=false`，不影响软件功能自动化评分，也不宣称已经完成。
