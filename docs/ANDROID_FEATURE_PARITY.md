...
核心功能基线由 `docs/ANDROID_CORE_BASELINE.json` schema v3 机器校验，分别记录软件功能、同提交自动化验收、真机兼容和生产签名状态。当前软件能力状态为 100%（18 项 full），但这不等于可正式发布：同提交 CI evidence、真实账号四平台真机证据和生产签名制品必须分别通过门禁，模拟器结果不能冒充 OEM、硬件编解码、真实账号或生产密钥证明。Web、Windows 与 Android 从 2.2.4 起共享同一语义版本。
...
| 平台导入与下载任务 | 已完成（自动化协议验收） | HTTP 回放和故障注入覆盖 200、206、416、403、429、500、缺失 Content-Length、不支持/错误 Range、连接中断和超时；登录回放覆盖等待、确认、过期和拒绝。正式发布还必须由 `-RequireDeviceEvidence` 校验当前提交、当前版本、真实设备身份及 Bilibili、抖音、快手、YouTube 四平台真实账号流程 |
...
| 软件升级与发行自动化门禁 | 已完成（门禁能力） | CI 在同一持久化 API 29 AVD 上从仓库 `v2.2.4` APK 覆盖安装当前 APK，并通过数据库迁移测试和独立持久化哨兵复验应用数据；证据只对 JSON 中记录的同一提交与版本有效。正式构建脚本强制生产密钥并归档 APK、SHA-256、证书指纹及 R8 资料；发布工作流同时强制真实设备四平台 evidence |
