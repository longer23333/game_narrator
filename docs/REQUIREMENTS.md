...
| FR-204 | P0 | 视觉结果质量校验与自恢复 | 已实现 | 空描述、低置信度或批次全零评分会先重试当前模型，再切换可用候选模型，最终以规则结果降级；逐帧尝试记录写入视觉分析产物 |
| FR-205 | P2 | 本地模型管理与切换 | 已实现 | 设置页实时列出 Ollama 已安装模型、健康状态、内存/显存估算和当前视觉/文案角色；推荐模型可直接下载，且只能切换到已安装模型，选择会立即成为本地推理运行时模型 |
